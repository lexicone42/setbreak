...
Features were added across 14 schema versions. Each tier builds on the previous.
...
### Spectral features (v1-v2)

Frequency-domain characteristics computed from STFT frames, aggregated as mean and standard deviation across all frames.
...
Mel-frequency cepstral coefficients — compact timbral representation. 13 coefficients (0-12), each stored as mean and standard deviation.
...
### Temporal & rhythm (v1-v2)

Beat tracking, onset detection, and rhythmic analysis.
...
Pitch detection (pYIN), key estimation, and chord analysis.
...
| `mode_clarity` | f64 | How clearly major vs minor (low = ambiguous) |
...
### Loudness & dynamics (v1-v3)

Perceptual loudness measurements per EBU R128.

| Column | Type | Description |
|--------|------|-------------|
| `lufs_integrated` | f64 | Integrated loudness (LUFS) |
...
### Structure & segments (v1-v4)

Structural analysis — how the track evolves over time.
...
### Per-frame derived features (v9-v11)

Higher-order features computed from frame-level data — slopes, correlations, entropy, and distributional statistics that capture how features evolve over time.
...
| `peak_energy_time` | f64 | Normalized position (0-1) of peak energy |
...
| `mfcc_flux_mean` | f64 | Rate of timbral change (mean MFCC delta) |
...
Detailed spectral decomposition — contrast across octave bands, tonal space, chromatic flux, and sub-band dynamics.
...
| `tonnetz_json` | text | 6D tonal centroid (Tonnetz) — harmonic space position (JSON) |
| `tonnetz_flux_mean` | f64 | Rate of movement through tonal space |
| `chroma_flux_mean` | f64 | Rate of chromatic change |

### Rhythm detail (v12)

Extended rhythmic analysis — beat patterns, syncopation, groove characteristics.

| Column | Type | Description |
|--------|------|-------------|
| `beat_pattern_json` | text | Beat-relative onset histogram (JSON array) |
| `syncopation` | f64 | Off-beat emphasis (high = syncopated rhythm) |
| `pulse_clarity` | f64 | How clear/strong the beat is (0 = no pulse, 1 = metronomic) |
| `offbeat_ratio` | f64 | Fraction of onsets on off-beats |

### Timbral & stereo (v13)

Psychoacoustic and spatial features — roughness, stereo field, MFCC dynamics, temporal modulation.
...
| `mfcc_delta_mean_json` | text | Rate of MFCC change over time — timbral dynamics (JSON, 13 coefficients) |
| `mfcc_delta_delta_mean_json` | text | Acceleration of timbral change (JSON, 13 coefficients) |
...
Higher-level features that capture musical concepts — harmonic vs percussive balance, tonal complexity, spectral texture, rhythmic shape, and structural variety.
...
| `chromagram_entropy` | f64 | Shannon entropy of mean 12-bin chroma. Low = simple tonality (one key center), high = chromatic/atonal. Max ≈ 2.485 |
| `spectral_contrast_slope` | f64 | Linear regression slope across 7 octave-band contrast means. Captures spectral texture tilt |
...
| `section_diversity_score` | f64 | Mean pairwise Euclidean distance between normalized segment feature vectors. High = track moves through many different textures |
